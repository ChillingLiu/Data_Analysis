...
def single_folder_fft(folder_name, n_channel, files_start_index, files_end_index):
	...
	# assume 0.000000 always exists
	if files_end_index > (len(files) - 1):
		print("Incorrect files_end_index!")
		exit()
	...
	for i in range(files_start_index, files_end_index + 1):
		file_name = folder_name + "/" + files[i]
		channels_average += single_file_fft(file_name, n_channel)

	channels_average = np.divide(channels_average, files_end_index - files_start_index + 1)

	# save channels_average_fft before return!
	save_name = folder_name.split("/")[1] + "_avg(" + str(files_start_index) + ", " + str(files_end_index) + ")_fft.txt"

	f = open(save_name, "w")
	f.write("x		CH1		CH2		CH3\n")
	for i in range(len(channels_average[0])):
		f.write(str(i) + "		")
		for j in range(n_channel):
			f.write(str(channels_average[j][i]) + "		")
		f.write("\n")
	f.close()
...
def fft_plot(folder_name, files_start_index, files_end_index, channels, n_channel, modulation_freq, n_modulation):
	...
	fig.suptitle(folder_name + ": avg(" + str(files_start_index) + ", " + str(files_end_index) + ")", color = "r")

	save_name = folder_name.split("/")[1] + "_avg(" + str(files_start_index) + ", " + str(files_end_index) + ")_fft.png"
...
folder_name = "sic-gold-intensity dep-10-08-2024/100uw"
n_channel = 3
modulation_freq = 1000
n_modulation = 4

# the files you want to take the avarage fft. 
files_start_index = 0 # indicating the file 0.000000
files_end_index = 2
...
channels_average = single_folder_fft(folder_name, n_channel, files_start_index, files_end_index)
fft_plot(folder_name, files_start_index, files_end_index, channels_average, n_channel, modulation_freq, n_modulation)
